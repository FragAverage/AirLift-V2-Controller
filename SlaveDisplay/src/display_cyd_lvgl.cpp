// ---------------------------------------------------------------------------
// cyd_lvgl env: same ILI9341 320x240 landscape SPI panel as env:cyd
// (ESP32-2432S028R "Cheap Yellow Display"), rendered with LVGL widgets
// instead of raw TFT_eSPI draw calls. Same on-screen layout as display.cpp on
// purpose -- see platformio.ini's cyd_lvgl comment and README.md's "The
// round21 board" section (round21_lvgl documents the general LVGL-backend
// approach this file follows too).
//
// Structurally this is display_round128_lvgl.cpp with the board swapped out
// -- the CYD's ILI9341 is just as well supported by TFT_eSPI as round128's
// GC9A01, so the same "keep TFT_eSPI for panel bring-up + pixel push, LVGL
// just owns everything drawn on top of it" approach applies unchanged,
// including the real PWM setBacklight() (GPIO21 here, same story as
// round128's GPIO40 -- not lcd147's input-only GPIO46).
//
...
// framebuffer; TFT_eSPI/the ILI9341 own that).
...
// --- widget helpers (identical to round21_lvgl's/round128_lvgl's) ---------
...
void buildGaugeScreen() {
  ...
  tankValueLabel = mkLabel(tankCell, &font_dseg7_22, COL_TANK);
...
void begin() {
  ...
  // fix lcd147_lvgl/round128_lvgl needed, confirmed there on real hardware
  // (colours came out with a purple/magenta cast instead of the intended
  // amber without it).
...
void setBacklight(uint8_t percent) {
  // GPIO21 on this board is a normal PWM-capable pin (this board's TFT_BL --
  // see platformio.ini's cyd env), same story as round128's GPIO40, unlike
  // lcd147's input-only GPIO46, so this backend drives real brightness
  // instead of lcd147_lvgl's always-on no-op.
