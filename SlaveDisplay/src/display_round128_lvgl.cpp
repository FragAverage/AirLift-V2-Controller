// ---------------------------------------------------------------------------
// round128_lvgl env: same GC9A01 240x240 round SPI panel as env:round128
// (Waveshare ESP32-S3-LCD-1.28, plain/non-touch), rendered with LVGL widgets
// instead of raw TFT_eSPI draw calls. Same on-screen layout as display.cpp on
// purpose -- see platformio.ini's round128_lvgl comment and README.md's "The
// round21 board" section (round21_lvgl documents the general LVGL-backend
// approach this file follows too).
//
// Structurally this is display_lcd147_lvgl.cpp with the board swapped out --
// round128's GC9A01 is just as well supported by TFT_eSPI as lcd147's
// ST7789, so the same "keep TFT_eSPI for panel bring-up + pixel push,
// LVGL just owns everything drawn on top of it" approach applies unchanged.
// The one real difference is setBacklight(): lcd147's GPIO46 backlight pin
// can't do PWM at all (input-only), so that board's backend leaves the
// panel permanently lit; round128's GPIO40 is a normal PWM-capable pin (see
// platformio.ini's round128 env comment for how that pin number was found),
// so this file drives it exactly like display.cpp's non-LCD147 branch does.
//
...
// PSRAM (this env doesn't request any -- see round21_lvgl's identical
// comment for why this is a short-lived render scratchpad, not the actual
// framebuffer; TFT_eSPI/the GC9A01 own that).
...
// --- widget helpers (identical to round21_lvgl's/lcd147_lvgl's) -----------
...
void begin() {
  ...
  // needs no rotation-specific logic at all. menu::begin() (called before
  ...
  // opposite (big-endian, MSB-first-over-SPI) unless told otherwise -- same
  // fix lcd147_lvgl needed, confirmed there on real hardware (colours came
  // out with a purple/magenta cast instead of the intended amber without it).
...
void setBacklight(uint8_t percent) {
  // GPIO40 on this board is a normal PWM-capable pin (confirmed on real
  // hardware -- see platformio.ini's round128 env comment), unlike lcd147's
  // input-only GPIO46, so this backend drives real brightness instead of
  // lcd147_lvgl's always-on no-op.
  if (percent > 100) percent = 100;
  const uint32_t maxDuty = (1u << BACKLIGHT_PWM_BITS) - 1;
  uint32_t duty = (maxDuty * percent) / 100;
  // The panel's BL line is active high on this board; invert if
  // TFT_BACKLIGHT_ON is ever changed to LOW.
